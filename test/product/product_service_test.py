...
from test_config import ProductStandardValues, ValidationRegexes
...
async def test_1_get_product_by_id_service() -> None:
    ...
    with pytest.raises(AppException) as app_exception:
        await get_product_by_id_service(ProductStandardValues.nonexistent_id)

    await assert_product_not_found(
        ProductStandardValues.nonexistent_id,
        app_exception,
    )
...
async def test_2_create_product_service() -> None:
    ...
    with pytest.raises(
        ...
        CreateProductSchema(
            name=ProductStandardValues.wrong_type_name,
            price_in_cents=ProductStandardValues.price_in_cents,
            user_access=ProductStandardValues.user_access,
        )
...
async def test_3_create_product_service() -> None:
    ...
    with pytest.raises(
            ValidationError,
            match=f"^1 validation error for CreateProductSchema\nprice_in_cents\n\\s+{ValidationRegexes.decimal_type_pattern}$",
    ):
        CreateProductSchema(
            name=ProductStandardValues.name,
            price_in_cents=ProductStandardValues.wrong_type_price_in_cents,
            user_access=ProductStandardValues.user_access,
        )
...
async def test_4_create_product_service() -> None:
    ...
    with pytest.raises(
        ...
        CreateProductSchema(
            name=ProductStandardValues.name,
            price_in_cents=ProductStandardValues.price_in_cents,
            user_access=ProductStandardValues.wrong_type_user_access,
        )
...
async def test_1_update_product_by_id_service() -> None:
    ...
    with pytest.raises(AppException) as app_exception:
        await update_product_by_id_service(
            ProductStandardValues.nonexistent_id, UpdateProductSchema(
                name=ProductStandardValues.updated_name,
                price_in_cents=ProductStandardValues.updated_price_in_cents,
                user_access=ProductStandardValues.updated_user_access,
        ))
...
async def test_5_update_product_by_id_service(standard_product: ProductModel) -> None:
    ...
    assert updated_product.name == ProductStandardValues.updated_name
    assert updated_product.price_in_cents == ProductStandardValues.updated_price_in_cents
    assert updated_product.user_access == ProductStandardValues.updated_user_access
...
async def test_1_delete_product_by_id_service() -> None:
    ...
    with pytest.raises(AppException) as app_exception:
        await delete_product_by_id_service(ProductStandardValues.nonexistent_id)
